...
def load_sensor_data(file_path: Path, sensor_type: str):
    # This print statement will appear in the terminal where 'streamlit run' is executing
    ...
    with ultrasonic_placeholder.container():
        ...
        if not df_ultrasonic.empty:
            ...
            if not all(col in df_ultrasonic.columns for col in required_us_cols):
                ...
                st.caption("Recent Ultrasonic Readings")  # Caption for the table
                st.dataframe(df_ultrasonic.tail(5).set_index('timestamp'), use_container_width=True)  # Table restored

                st.caption("Ultrasonic Distance Over Time")  # Caption for the chart
                st.line_chart(df_ultrasonic.set_index('timestamp')[['distance_mm']].tail(100), use_container_width=True)
        else:
    ...
    for sensor_id_key, inductive_csv_filename in INDUCTIVE_SENSOR_FILES.items():
        ...
        with inductive_placeholders[sensor_id_key].container():
            ...
            if not df_inductive.empty:
                required_ind_cols = ['timestamp', 'sensor_id', 'distance_to_target_mm', 'output_state',
                                     'switching_function']
                if not all(col in df_inductive.columns for col in required_ind_cols):
                    ...
                    ind_cols[3].metric("Output", "DETECTED" if latest_ind_point['output_state'] == (
                        1 if latest_ind_point['switching_function'] == "NO" else 0) else "NOT DETECTED")
                    ind_cols[4].metric("Switch Func", str(latest_ind_point['switching_function']))

                    st.caption(f"Recent Inductive Readings ({sensor_id_key})")  # Caption for the table
                    st.dataframe(df_inductive.tail(5).set_index('timestamp'),
                                 use_container_width=True)  # Table restored
            else:
    ...
    for sensor_id_key, heat_csv_filename in HEAT_SENSOR_FILES.items():
        ...
        with heat_sensor_placeholders[sensor_id_key].container():
            ...
            if not df_heat.empty:
                required_heat_cols = ["timestamp", "sensor_id", "simulated_material_temp_c", "fire_alarm_state",
                                      "fault_state", "green_led_normal_status", "red_led_trip_status"]
                if not all(col in df_heat.columns for col in required_heat_cols):
                    ...
                    hs_cols[3].metric("Fire Status", fire_status,
                                      delta_color=("inverse" if fire_status == "ALARM" else "off"))

                    fault_status = "FAULT" if latest_heat_point['fault_state'] == 1 else "OK"
                    hs_cols[4].metric("Sensor Fault", fault_status,
                                      delta_color=("inverse" if fault_status == "FAULT" else "off"))
                    ...
                    if not led_status_text: led_status_text.append("LEDs Off")  # Should not happen if logic is correct
                    hs_cols[5].markdown(f"**LEDs:** {', '.join(led_status_text)}")

                    st.caption(f"Recent Heat Sensor Readings ({sensor_id_key})")  # Caption for the table
                    st.dataframe(df_heat.tail(5).set_index('timestamp'), use_container_width=True)  # Table restored
            else:
